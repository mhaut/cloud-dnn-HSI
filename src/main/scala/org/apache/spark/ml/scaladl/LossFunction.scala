...
import breeze.linalg.{csvwrite, sum => Bsum, CSCMatrix, DenseMatrix => BDM, DenseVector => BDV}
import breeze.numerics.{log => brzlog}

/**
 * Trait for loss function
 */
private[scaladl] trait LossFunction {
  /**
...
private[scaladl] class SigmoidLayerWithSquaredError extends Layer {
...
private[scaladl] class SigmoidLayerModelWithSquaredError
...
private[scaladl] class SoftmaxLayerWithCrossEntropyLoss extends Layer {
  override val weightSize = 0
  override val inPlace = true
...
private[scaladl] class SoftmaxLayerModelWithCrossEntropyLoss extends LayerModel with LossFunction {
...
private[scaladl] class EmptyLayerWithSquaredError extends Layer {
  override val weightSize = 0
  override def getOutputSize(inputSize: Int): Int = inputSize
  override val inPlace = true
  override def createModel(weights: BDV[Double]): LayerModel =
...
private[scaladl] class EmptyLayerModelWithSquaredError extends LayerModel with LossFunction {

  val weights = new BDV[Double](0)

  override def loss(output: BDM[Double], target: BDM[Double], delta: BDM[Double]): Double = {
    ApplyInPlace(output, target, delta, (o: Double, t: Double) => o - t)
    Bsum(delta :* delta) / (2 * output.cols * output.rows)
  }
