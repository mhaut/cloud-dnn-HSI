...
package org.apache.spark.mllib.scaladl.optimization
...
class LBFGS(private var gradient: Gradient, private var updater: Updater)
  ...
  private var convergenceTol = Double.MinPositiveValue
...
object LBFGS extends Logging {
  ...
  def runLBFGS(
    ...
    while (states.hasNext) {
      ...
      state = states.next()
      logWarning("LBFGS.runLBFGS finished iteration %s. Error: %s".format(
       iter.toString(), lossHistory.result().takeRight(1).mkString("")))
      iter = iter + 1
    }
    lossHistory += state.value

    val stop = DateTime.now(DateTimeZone.UTC).getMillis()
    val time = stop - start
...
// scalastyle:off
    logWarning("LBFGS.runLBFGS finished. Last training loss %s".format(
      lossHistoryArray.takeRight(1).mkString(", ")))
    logWarning("Total elapsed time: %d ms".format(time))
